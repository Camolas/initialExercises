...
{// veicule é uma thread que contém um fifo
...
void * tcontroller_N(void * avg)
{
  ...
  while(read(fd, &vehicle, sizeof(vehicle)) != 0)
  {
     ...
     {
        pthread_t tid;
        pthread_create(&tid, NULL, tpark_helper, &vehicle); // fazer um open para este mesmo fifo com 1 open por wronly
        fd = open(FIFON, O_WRONLY); // car_assistant
     } // significa que chega um carro para fechar o parque..
  }// reservo um id dos automoveis para reservar o parque!
// parque aberto durante um certo periodo sleep (duration)
// algures se recebo 1 veículo c/ um id = 0 tenho: de fechar o parque
